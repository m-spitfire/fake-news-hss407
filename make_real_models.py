...
pickle.dump(data_lemmatized, open("data_lemmatized_real.pkl", "wb"))

dirichlet_dict = corpora.Dictionary(data_lemmatized)
bow_corpus = [dirichlet_dict.doc2bow(text) for text in data_lemmatized]
pickle.dump(bow_corpus, open('corpus_real.pkl', 'wb'))
dirichlet_dict.save('dictionary_real.gensim')
...
for i in tqdm(num_topics):
    LDA_models[i] = LdaMulticore(corpus=bow_corpus,
                            ...
                            # TODO: increase passes
                             ...
                             workers=15,
                             random_state=42)
    LDA_models[i].save(f"models_real/{i}_multi_symm.gensim")
